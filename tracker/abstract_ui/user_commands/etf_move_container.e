...
feature -- command
	...
	move_container (cid: STRING; pid1: STRING; pid2: STRING)
		...
		do
			...
			elseif not model.get_phases.has (pid2) or not model.get_phases.has (pid1) then
				error := (msg.phase_id_non_existent)
			elseif attached model.get_phase_containing_cid (cid) as phase and then phase.get_pid /~ pid1 then
				error := (msg.cont_id_not_in_source)
			elseif not model.under_capacity (pid2) then
				error := (msg.cont_exceeds_cap)
			elseif attached model.get_container (cid) as cont and then model.cont_gt_max_phase_rad (cont.get_rad, pid2) then
				error := (msg.cont_exceeds_rad)
			elseif attached model.get_container (cid) as cont and then not model.phase_expects_mat (cont.get_material, pid2) then
